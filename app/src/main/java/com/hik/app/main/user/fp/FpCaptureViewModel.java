 ...
 **/
public class FpCaptureViewModel extends ViewModel {
    private static final String TAG = "FpCaptureViewModel";
     ...
     */
    void startFpCapture() {
        ApiHelper.INSTANCE.fpCapture(new ApiCall<CaptureFingerPrint>() {
            @Override
            public void onSuccess(CaptureFingerPrint captureFingerPrint) {
                KLog.e(TAG, "fpCapture OK. fingerNo: " + captureFingerPrint.getFingerNo());
                go_back.postValue(true);
            }

            @Override
            public void onError() {
                KLog.e(TAG, "fpCapture onError");
                go_back.postValue(false);
            }
        }, new CaptureFingerPrintCond(2));
    }
}
